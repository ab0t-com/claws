...
func TestIntegration_CreateWithImage(t *testing.T) {
	root := t.TempDir()
	clawctl(t, root, "create", "alpha", "--image=openclaw:custom")
...
func TestIntegration_ImagePin(t *testing.T) {
	...
	_, err := clawctl(t, root, "image", "pin", "alpha", "openclaw:v1")
...
func TestIntegration_ImagePinPolicyBlock(t *testing.T) {
	...
	_, err := clawctl(t, root, "image", "pin", "alpha", "evil:latest")
	if err == nil {
...
func TestIntegration_CreateWithImagePolicyBlock(t *testing.T) {
	...
	_, err := clawctl(t, root, "create", "alpha", "--image=evil:latest")
	if err == nil {
...
func TestIntegration_UpgradeRequiresInstance(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "upgrade", "nonexistent")
	if err == nil {
...
func TestIntegration_ImageListHelp(t *testing.T) {
	root := t.TempDir()
	out, _ := clawctl(t, root, "image", "--help")
...
func TestIntegration_UpgradeHelp(t *testing.T) {
	root := t.TempDir()
	out, _ := clawctl(t, root, "upgrade", "--help")
