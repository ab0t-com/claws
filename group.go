...
func cmdGroupAdd(args []string) error {
	...
	for _, instanceName := range args[1:] {
		...
		if wasRunning {
			...
			dcRun(paths, newName, "up", "-d", "openclaw-gateway")
		}
...
func rebuildGroupOverride(paths Paths, ref InstanceRef) error {
	...
	if f.Skills {
		mount := yamlVolume(6, filepath.Join(sharedBase, "skills"), "/home/node/.openclaw/bundled-skills", "ro")
	...
	if f.Workspace {
		mount := yamlVolume(6, filepath.Join(sharedBase, "workspace"), "/home/node/.openclaw/shared", "rw")
		gwVolumes = append(gwVolumes, mount)
		cliVolumes = append(cliVolumes, mount)
	}
	if f.Hooks {
		mount := yamlVolume(6, filepath.Join(sharedBase, "hooks"), "/home/node/.openclaw/shared-hooks", "ro")
		gwVolumes = append(gwVolumes, mount)
		cliVolumes = append(cliVolumes, mount)
	}
	...
	if role == "manager" && ref.Group != "" {
		...
		gwVolumes = append(gwVolumes, yamlVolume(6, td, "/home/node/.openclaw/tasks", "rw"))
		...
		for _, m := range members {
			...
			if memberRole == "worker" {
				...
				gwVolumes = append(gwVolumes, yamlVolume(6, workerWs, "/home/node/.openclaw/workers/"+m, "ro"))
			}
		}
		...
		gwVolumes = append(gwVolumes, yamlVolume(6, outputDir, "/home/node/.openclaw/output", "rw"))
	}

	// Worker role: read-only task feed + write to output
	if role == "worker" && ref.Group != "" {
		td := filepath.Join(GroupSharedDir(paths, ref.Group), "tasks")
		gwVolumes = append(gwVolumes, yamlVolume(6, td, "/home/node/.openclaw/tasks", "ro"))
		...
		if managerName != "" {
			...
			if _, err := os.Stat(managerWs); err == nil {
				gwVolumes = append(gwVolumes, yamlVolume(6, managerWs, "/home/node/.openclaw/manager", "ro"))
			}
		}
	}
