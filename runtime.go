...
// interface by providing a Runtime struct. clawctl uses these values instead
...
// RuntimeCapabilities declares which clawctl features this runtime supports.
...
// Reads CLAWCTL_RUNTIME from instance.env, defaults to "openclaw".
...
func resolveRuntime(paths Paths, instanceName string) (Runtime, error) {
	...
	rtName := readEnvValue(envFile, "CLAWCTL_RUNTIME")
	if rtName == "" {
		return openclawRuntime(), nil // no runtime specified → default
	}
	rt, ok := getRuntimeByName(paths, rtName)
	if !ok {
		return Runtime{}, fmt.Errorf("runtime '%s' not found (referenced by instance '%s'). Register it: clawctl runtime add %s --image=...", rtName, instanceName, rtName)
	}
	return rt, nil
}
...
// ---------------------------------------------------------------------------
// clawctl runtime — manage runtime definitions
// ---------------------------------------------------------------------------

func cmdRuntime(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime <list|show|add|remove>")
	}
	switch args[0] {
	case "list", "ls":
...
func cmdRuntimeShow(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime show <name>")
	}
	paths := resolvePaths()
	name := args[0]
...
func cmdRuntimeAdd(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime add <name> --image=<image> [--health=<endpoint>] [--port=<port>] [--gateway-service=<name>]")
	}
	...
	if fromName != "" {
		...
		if !ok {
			return errorf("base runtime '%s' not found — see available: clawctl runtime list", fromName)
		}
		rt = base
		rt.Name = name
	...
	fmt.Println()
	fmt.Printf("  Use: clawctl create <name> --runtime=%s\n", name)
	fmt.Println()
...
func cmdRuntimeRemove(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime remove <name>")
	}
	paths := resolvePaths()
	name := args[0]
...
func cmdRuntimeInit(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime init <name>")
	}
	paths := resolvePaths()
	name := args[0]
...
# Documentation: clawctl runtime --help
services:
  %s-gateway:
...
`, name, name, name)
	...
	fmt.Printf("    3. Test it: clawctl runtime test %s\n", name)
	fmt.Printf("    4. Create an instance: clawctl create my-agent --runtime=%s\n", name)
	return nil
}
...
func cmdRuntimeTest(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime test <name>")
	}
	paths := resolvePaths()
	name := args[0]
	...
	rt, ok := getRuntimeByName(paths, name)
	if !ok {
		return errorf("runtime '%s' not found — register it first: clawctl runtime add %s --image=...", name, name)
	}
	...
	if _, err := os.Stat(composePath); err != nil {
		...
		fmt.Printf("    Create it: clawctl runtime init %s\n", name)
	...
	fmt.Println()
	info(fmt.Sprintf("Runtime '%s' looks valid. Create an instance: clawctl create <name> --runtime=%s", name, name))
	return nil
}
...
func cmdRuntimeExport(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime export <name>")
	}
	paths := resolvePaths()
	name := args[0]
...
func cmdRuntimeImport(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime import <file.json>")
	}
	paths := resolvePaths()
	filePath := args[0]
...
func cmdRuntimeDetect(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl runtime detect <image:tag>")
	}
	image := args[0]
	...
	if isNode && contains(entrypoint, "dist/index.js") {
		...
		fmt.Printf("    clawctl create my-agent --image=%s\n", image)
		fmt.Println()
		fmt.Println("  Or if it has differences (health endpoint, port):")
		fmt.Printf("    clawctl runtime add %s --from=openclaw --image=%s\n", name, image)
	} else {
		...
		fmt.Printf("    clawctl runtime add %s --image=%s --port=%s --health=/health --container-home=%s --no-channels --no-cli\n",
			name, image, port, home)
		fmt.Println()
		fmt.Println("  Or scaffold and customize:")
		fmt.Printf("    clawctl runtime init %s\n", name)
	}
