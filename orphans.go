...
// ---------------------------------------------------------------------------
// clawctl orphans — drift detection between Docker state and clawctl's
// registry. Surfaces containers that match clawctl's naming convention but
...
// container name. clawctl-managed containers always look like
...
func cmdOrphansList(args []string) error {
	...
	fmt.Printf("%sOrphan containers (%d) — running on the host but not in clawctl's registry%s\n\n", bold, len(orphans), nc)
	for _, o := range orphans {
		...
		fmt.Printf("    %sclean:%s    clawctl orphans clean %s\n", bold, nc, o.Container)
		fmt.Println()
	}
	fmt.Printf("  Or remove all at once: clawctl orphans clean --all [--yes]\n")
	return nil
}
...
func cmdOrphansClean(args []string) error {
	...
	if !all && target == "" {
		return errorf("usage: clawctl orphans clean <container> | clawctl orphans clean --all [--yes]")
	}
	if all && target != "" {
	...
	if all {
		...
		if len(toRemove) == 0 {
			return errorf("no orphan container named '%s' — run: clawctl orphans", target)
		}
	}
