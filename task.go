...
// ---------------------------------------------------------------------------
// clawctl task — task lifecycle management
// ---------------------------------------------------------------------------

func cmdTask(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl task <create|list|claim|complete|status> [args...]")
	}
	switch args[0] {
	case "create":
...
func cmdTaskCreate(args []string) error {
	if len(args) < 2 {
		return errorf("usage: clawctl task create <group> <title> [--from=<instance>] [--description=<text>]")
	}
...
func cmdTaskClaim(args []string) error {
	if len(args) < 3 {
		return errorf("usage: clawctl task claim <group> <task-id> --by=<instance>")
	}
...
func cmdTaskComplete(args []string) error {
	if len(args) < 2 {
		return errorf("usage: clawctl task complete <group> <task-id> [--result=<text>]")
	}
...
func cmdTaskStatus(args []string) error {
	if len(args) < 2 {
		return errorf("usage: clawctl task status <group> <task-id>")
	}
