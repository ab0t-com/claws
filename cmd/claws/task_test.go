...
func TestIntegration_TaskCreate(t *testing.T) {
	...
	out, err := claws(t, root, "task", "create", "team", "summarize-report", "--from=lead")
	if err != nil {
...
func TestIntegration_TaskClaim(t *testing.T) {
	...
	out, _ := claws(t, root, "task", "create", "team", "do-thing", "--from=lead")
	taskID := extractTaskID(t, out)

	out, err := claws(t, root, "task", "claim", "team", taskID, "--by=dev1")
	if err != nil {
...
func TestIntegration_TaskComplete(t *testing.T) {
	...
	out, _ := claws(t, root, "task", "create", "team", "build-feature", "--from=lead")
	taskID := extractTaskID(t, out)
	claws(t, root, "task", "claim", "team", taskID, "--by=dev1")

	out, err := claws(t, root, "task", "complete", "team", taskID, "--result=done-and-tested")
	if err != nil {
...
func TestIntegration_TaskList(t *testing.T) {
	...
	claws(t, root, "task", "create", "team", "task-one", "--from=lead")
	out2, _ := claws(t, root, "task", "create", "team", "task-two", "--from=lead")
	taskID2 := extractTaskID(t, out2)
	claws(t, root, "task", "claim", "team", taskID2, "--by=dev1")

	out, err := claws(t, root, "task", "list", "team")
	if err != nil {
...
func TestIntegration_TaskListFilterStatus(t *testing.T) {
	...
	claws(t, root, "task", "create", "team", "pending-task", "--from=lead")
	out2, _ := claws(t, root, "task", "create", "team", "claimed-task", "--from=lead")
	taskID2 := extractTaskID(t, out2)
	claws(t, root, "task", "claim", "team", taskID2, "--by=dev1")

	out, _ := claws(t, root, "task", "list", "team", "--status=pending")
...
func TestIntegration_TaskClaimNonexistent(t *testing.T) {
	...
	_, err := claws(t, root, "task", "claim", "team", "nonexistent-id", "--by=dev1")
	if err == nil {
...
func TestIntegration_TaskDoubleClaimFails(t *testing.T) {
	...
	out, _ := claws(t, root, "task", "create", "team", "contested-task", "--from=lead")
	taskID := extractTaskID(t, out)

	// First claim succeeds
	_, err := claws(t, root, "task", "claim", "team", taskID, "--by=dev1")
	if err != nil {
...
func TestIntegration_TaskStatus(t *testing.T) {
	...
	out, _ := claws(t, root, "task", "create", "team", "status-check", "--from=lead", "--description=test-desc")
	taskID := extractTaskID(t, out)

	out, err := claws(t, root, "task", "status", "team", taskID)
	if err != nil {
...
func TestIntegration_TaskLifecycle(t *testing.T) {
	...
	// Create
	out, _ := claws(t, root, "task", "create", "team", "full-lifecycle", "--from=lead")
	taskID := extractTaskID(t, out)

	// List — should be pending
	out, _ = claws(t, root, "task", "list", "team")
	if !strings.Contains(out, "pending") {
	...
	// Complete
	claws(t, root, "task", "complete", "team", taskID, "--result=all-done")

	// Status — should be done
	out, _ = claws(t, root, "task", "status", "team", taskID)
	if !strings.Contains(out, "all-done") {
		t.Error("should show result")
	}

	// List done
	out, _ = claws(t, root, "task", "list", "team", "--status=done")
