...
// ---------------------------------------------------------------------------
// claws errors — incident-triage umbrella view.
//
...
//   3. Recent claws operations that returned error (filtered audit log)
...
func computeFixPaths(rep errorsReport) []string {
	...
	for _, c := range rep.Containers {
		if c.State == "restarting" {
			add(fmt.Sprintf("claws logs %s --tail=50   # diagnose restart loop", c.Name))
		}
	}
	...
	for name := range loggedInstances {
		add(fmt.Sprintf("claws logs %s --grep=error --since=2h", name))
	}

	// Orphans get a one-shot cleanup command (only suggest once).
	if len(rep.Orphans) > 0 {
		add("claws orphans clean --all --yes")
	}
...
func renderErrorsReport(rep errorsReport, filterGroup string, since time.Duration) {
	...
	fmt.Printf("%sclaws errors%s%s — last %s\n\n", bold, scope, nc, since)
	...
	// Section 3: claws audit errors
	fmt.Printf("%sRecent claws operations that returned error (%d)%s\n", bold, len(rep.AuditErrors), nc)
