...
func TestIntegration_AccessInit(t *testing.T) {
	root := t.TempDir()
	out, err := claws(t, root, "access", "init")
	if err != nil {
...
func TestIntegration_AccessGrant(t *testing.T) {
	root := t.TempDir()
	claws(t, root, "access", "init")
	out, err := claws(t, root, "access", "grant", "bob", "operator")
	if err != nil {
		t.Fatalf("grant failed: %v\n%s", err, out)
	}

	out, _ = claws(t, root, "access", "show")
...
func TestIntegration_AccessRevoke(t *testing.T) {
	root := t.TempDir()
	claws(t, root, "access", "init")
	claws(t, root, "access", "grant", "bob", "operator")
	claws(t, root, "access", "revoke", "bob")
...
func TestIntegration_TokenRotate(t *testing.T) {
	...
	out, err := claws(t, root, "token", "rotate", "alpha")
	if err != nil {
...
func TestIntegration_TokenShow(t *testing.T) {
	...
	out, err := claws(t, root, "token", "show", "alpha")
	if err != nil {
