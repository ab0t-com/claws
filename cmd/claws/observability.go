...
// ---------------------------------------------------------------------------
// claws channels (no args) — fleet-wide channel matrix
// claws auth status [name]   — per-agent auth provider inventory
//
...
// sync with the channels claws actually knows how to provision. Any
// channel configured on an agent outside this set still shows up via
// `claws channel status <name>`, just not in the fleet matrix.
...
func cmdChannelsMatrix(args []string) error {
	...
	fmt.Println("  Drill in: claws channel status <name>  |  claws channel security <name>")
	return nil
}
...
func cmdAuthStatus(args []string) error {
	...
	fmt.Println("  Last auth is the most recent `claws auth <name> ...` invocation from the audit log.")
	if probe {
...
func verifyOneInstance(paths Paths, name string) authVerifyResult {
	...
	if port == "" {
		...
		res.FixCommand = fmt.Sprintf("claws start %s", name)
		return res
	}
...
func suggestReauthCommand(name, provider string) string {
	...
	case "openai-codex", "codex", "openai":
		return fmt.Sprintf("claws auth %s codex", name)
	case "anthropic", "claude":
		return fmt.Sprintf("claws auth %s apikey anthropic <key>", name)
	default:
		return fmt.Sprintf("claws auth %s codex  (or: claws auth %s apikey <provider> <key>)", name, name)
	}
}
...
func cmdAuthVerify(args []string) error {
	...
	name := firstPositional(args)
	if name == "" {
		return errorf("usage: claws auth verify <name> [--json]")
	}
	if err := requireInstance(paths, name); err != nil {
		return err
	}
