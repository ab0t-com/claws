...
func cmdToken(args []string) error {
	if len(args) < 1 {
		return errorf("usage: claws token <rotate|show> <instance>")
	}
	switch args[0] {
	case "rotate":
...
func cmdTokenRotate(args []string) error {
	...
	if name == "" {
		return errorf("usage: claws token rotate <instance> | claws token rotate --group=<name> [--yes]")
	}
...
func cmdTokenShow(args []string) error {
	if len(args) < 1 {
		return errorf("usage: claws token show <instance>")
	}
	paths := resolvePaths()
	name := args[0]
	if err := requireInstance(paths, name); err != nil {
...
// ---------------------------------------------------------------------------
// claws access — manage access control
// ---------------------------------------------------------------------------

func cmdAccess(args []string) error {
	if len(args) < 1 {
		return errorf("usage: claws access <init|show|grant|revoke|audit|tail>")
	}
	switch args[0] {
	case "init":
...
func cmdAccessInit(args []string) error {
	...
	fmt.Println()
	fmt.Println("  Grant access:  claws access grant <user> <admin|operator|user>")
...
func cmdAccessShow(args []string) error {
	...
	if !accessExists(paths) {
		fmt.Println("No access control configured. Run: claws access init")
		return nil
	}
	ac := readAccessConfig(paths)
...
func cmdAccessGrant(args []string) error {
	if len(args) < 2 {
		return errorf("usage: claws access grant <username> <admin|operator|user>")
	}
	paths := resolvePaths()
	username := args[0]
	roleName := args[1]
	...
	if !accessExists(paths) {
		return errorf("no access control configured — run: claws access init")
	}
...
func cmdAccessRevoke(args []string) error {
	if len(args) < 1 {
		return errorf("usage: claws access revoke <username>")
	}
	paths := resolvePaths()
	username := args[0]
...
func cmdAccessAudit(args []string) error {
	...
	data, err := os.ReadFile(logPath)
	if err != nil {
		fmt.Println("No audit log found. Enable with: claws policy init (sets auditLog: true)")
		return nil
	}
