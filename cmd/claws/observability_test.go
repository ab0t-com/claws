...
func TestSuggestReauthCommand(t *testing.T) {
	cases := []struct {
		...
		{"team/sarah", "openai-codex", "claws auth team/sarah codex"},
		{"alpha", "codex", "claws auth alpha codex"},
		{"team1/ben", "anthropic", "claws auth team1/ben apikey anthropic <key>"},
		{"team1/ben", "claude", "claws auth team1/ben apikey anthropic <key>"},
		// Unknown provider falls through to the generic form.
		{"alpha", "", "claws auth alpha codex  (or: claws auth alpha apikey <provider> <key>)"},
		{"alpha", "mistral", "claws auth alpha codex  (or: claws auth alpha apikey <provider> <key>)"},
	}
