...
// ---------------------------------------------------------------------------
// clawctl setup — guided interactive onboarding
// ---------------------------------------------------------------------------
...
func cmdSetup(args []string) error {
	...
	// Welcome
	// -----------------------------------------------------------------------
	fmt.Printf("\n  %sWelcome to clawctl%s — AI agent team manager.\n\n", bold, nc)
	...
	for {
		...
		if currentAuth == "codex" {
			...
			if err := cmdAuth([]string{fullName, "codex"}); err != nil {
				warn(fmt.Sprintf("Auth failed: %v — you can retry later: clawctl auth %s codex", err, fullName))
			} else {
		...
		if currentChannel != "" && currentChannel != "skip" {
			...
			if err := cmdChannel(channelArgs); err != nil {
				warn(fmt.Sprintf("Channel setup failed: %v — retry later: clawctl channel add %s %s", err, fullName, currentChannel))
			} else {
		...
		if err := cmdStart([]string{fullName}); err != nil {
			warn(fmt.Sprintf("Start failed: %v — start manually: clawctl start %s", err, fullName))
		} else {
			...
			if currentChannel != "" && currentChannel != "skip" {
				...
				fmt.Println()
				fmt.Printf("    Approve the pairing code: clawctl approve %s %s <CODE>\n", fullName, currentChannel)
			}
		}
		fmt.Println()
	...
	fmt.Printf("    clawctl list              — see all agents\n")
	fmt.Printf("    clawctl status            — system overview\n")
	fmt.Printf("    clawctl dashboard         — live status view\n")
	fmt.Printf("    clawctl audit             — security check\n")
	fmt.Printf("    clawctl setup             — add more agents\n")
	fmt.Println()
