...
// ---------------------------------------------------------------------------
// clawctl drift — umbrella over orphans (forward) + orphans --reverse +
...
func cmdDrift(args []string) error {
	...
	fmt.Printf("%sclawctl drift%s — state consistency check\n\n", bold, nc)
	...
	if len(rep.Forward) > 0 {
		fmt.Println("  clawctl orphans clean --all --yes")
	}
	for _, r := range rep.Reverse {
		if _, err := os.Stat(r.InstanceDir); err == nil {
			fmt.Printf("  clawctl start %s\n", r.Name)
		} else {
			fmt.Printf("  %sclawctl remove %s --purge --yes%s    # instance dir is missing\n", yellow, r.Name, nc)
		}
	}
	for _, d := range rep.DiskDrift {
	...
	for _, e := range rep.RegistryDrift {
		fmt.Printf("  clawctl remove %s --purge --yes    # registry-only entry, no data on disk\n", e.Name)
	}
	return nil
}
