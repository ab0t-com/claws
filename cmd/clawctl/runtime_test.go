...
func TestResolveRuntimeFromEnv(t *testing.T) {
	...
	os.WriteFile(filepath.Join(dir, "instance.env"), []byte("CLAWCTL_RUNTIME=custom\n"), 0600)
...
func TestResolveRuntimeBrokenReference(t *testing.T) {
	...
	dir := filepath.Join(paths.Root, "alpha")
	os.MkdirAll(dir, 0755)
	os.WriteFile(filepath.Join(dir, "instance.env"), []byte("CLAWCTL_RUNTIME=nonexistent-runtime\n"), 0600)
...
func TestMustResolveRuntimeFallback(t *testing.T) {
	...
	dir := filepath.Join(paths.Root, "alpha")
	os.MkdirAll(dir, 0755)
	os.WriteFile(filepath.Join(dir, "instance.env"), []byte("CLAWCTL_RUNTIME=broken\n"), 0600)
...
func TestIntegration_RuntimeList(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "runtime", "list")
	if err != nil {
...
func TestIntegration_RuntimeShow(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "runtime", "show", "openclaw")
	if err != nil {
...
func TestIntegration_RuntimeShowJson(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "runtime", "show", "openclaw", "--json")
	if err != nil {
...
func TestIntegration_RuntimeAdd(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "runtime", "add", "my-agent", "--image=my-agent:v1", "--health=/health", "--no-channels", "--no-pairing")
	if err != nil {
	...
	out, _ = clawctl(t, root, "runtime", "list")
...
func TestIntegration_RuntimeRemove(t *testing.T) {
	root := t.TempDir()
	clawctl(t, root, "runtime", "add", "temp-agent", "--image=temp:v1")
	out, err := clawctl(t, root, "runtime", "remove", "temp-agent")
	if err != nil {
...
func TestIntegration_RuntimeCannotRemoveBuiltin(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "runtime", "remove", "openclaw")
	if err == nil {
...
func TestIntegration_CreateStoresRuntime(t *testing.T) {
	...
	rtName := readEnvFromFile(t, envFile, "CLAWCTL_RUNTIME")
	if rtName != "openclaw" {
...
func TestIntegration_CreateWithCustomRuntime(t *testing.T) {
	...
	clawctl(t, root, "runtime", "add", "myagent", "--image=myagent:v1", "--no-channels")

	out, err := clawctl(t, root, "create", "alpha", "--runtime=myagent")
	if err != nil {
...
func TestIntegration_CreateWithUnknownRuntime(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "create", "alpha", "--runtime=nonexistent")
	if err == nil {
...
func TestIntegration_RuntimeAddFrom(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "runtime", "add", "nemoclaw", "--from=openclaw", "--image=nemoclaw:latest")
	if err != nil {
		t.Fatalf("runtime add --from failed: %v\n%s", err, out)
	}

	// Show it and verify inherited fields
	out, _ = clawctl(t, root, "runtime", "show", "nemoclaw", "--json")
	var rt Runtime
...
func TestIntegration_RuntimeAddFromOverrideHealth(t *testing.T) {
	root := t.TempDir()
	clawctl(t, root, "runtime", "add", "nanoclaw", "--from=openclaw", "--image=nanoclaw:v1", "--health=/status")

	out, _ := clawctl(t, root, "runtime", "show", "nanoclaw", "--json")
	var rt Runtime
	json.Unmarshal([]byte(out), &rt)
...
func TestIntegration_RuntimeAddFromNonexistent(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "runtime", "add", "bad", "--from=nonexistent")
	if err == nil {
...
func TestIntegration_RuntimeInit(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "runtime", "init", "my-agent")
	if err != nil {
...
func TestIntegration_RuntimeInitIdempotent(t *testing.T) {
	root := t.TempDir()
	clawctl(t, root, "runtime", "init", "my-agent")
	// Second init should fail without --force
	_, err := clawctl(t, root, "runtime", "init", "my-agent")
	if err == nil {
...
func TestIntegration_RuntimeTestOpenclaw(t *testing.T) {
	root := t.TempDir()
	out, _ := clawctl(t, root, "runtime", "test", "openclaw")
...
func TestIntegration_RuntimeTestNonexistent(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "runtime", "test", "nonexistent")
	if err == nil {
...
func TestIntegration_RuntimeExportImport(t *testing.T) {
	root := t.TempDir()
	// Register a runtime
	clawctl(t, root, "runtime", "add", "test-rt", "--image=test:v1", "--health=/up", "--no-channels")

	// Export
	out, err := clawctl(t, root, "runtime", "export", "test-rt")
	if err != nil {
	...
	// Remove original
	clawctl(t, root, "runtime", "remove", "test-rt")

	// Import
	out, err = clawctl(t, root, "runtime", "import", exportFile)
	if err != nil {
		t.Fatalf("import failed: %v\n%s", err, out)
	}

	// Verify it's back
	out, _ = clawctl(t, root, "runtime", "show", "test-rt", "--json")
	var rt Runtime
	json.Unmarshal([]byte(out), &rt)
...
func TestIntegration_RuntimeDetect(t *testing.T) {
	...
	out, _ := clawctl(t, root, "runtime", "detect", "openclaw:local")
...
func TestIntegration_RuntimeDetectNonexistent(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "runtime", "detect", "nonexistent-image:v999")
	if err == nil {
...
func TestIntegration_RuntimeCannotOverrideBuiltin(t *testing.T) {
	root := t.TempDir()
	_, err := clawctl(t, root, "runtime", "add", "openclaw", "--image=fake:v1")
	if err == nil {
