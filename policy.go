...
// ---------------------------------------------------------------------------
// clawctl policy — view and manage admin policy
// ---------------------------------------------------------------------------

func cmdPolicy(args []string) error {
	if len(args) < 1 {
		return errorf("usage: clawctl policy <show|init|validate|enforce>")
	}
	switch args[0] {
	case "show":
...
func cmdPolicyShow(args []string) error {
	...
	if !policyExists(paths) {
		fmt.Println("No policy configured. Run: clawctl policy init")
		return nil
	}
	p := readPolicy(paths)
...
func cmdPolicyInit(args []string) error {
	...
	fmt.Println(string(data))
	fmt.Println()
	fmt.Println("  Edit to customize: clawctl config edit policy")
	fmt.Println("  Validate instances: clawctl policy validate")
	return nil
}
