...
1. **"What are these agents actually on?"** — to discover that `team1/ben` was on `anthropic/claude-opus-4-6` and the rest were on `openai-codex/gpt-5.4`, I had to run `claws config get` four times *and then* cross-check against the gateway's startup log line, because the model isn't surfaced by any list/status command. The operator didn't remember setting Ben to Anthropic; if the CLI had told them at a glance, they wouldn't have had to remember.

2. **"Is this thing on Telegram or WhatsApp?"** — same shape: `claws channel status <name>` four times. No fleet view.

3. **"There's a container my CLI doesn't know about."** — `bob` was an orphan from a test run on 2026-05-20, restart-looping 1500+ times against deleted mounts. Nothing in claws tells you a container with the `openclaw-` project prefix exists outside the port registry. You only find it via `docker ps` — i.e., by going around the CLI.
...
## A day in the life of a claws operator (what's hard today)
...
| "Restart just team1 because something flaked" | Loop `claws restart team1/ben` per member, manually | No `--group=`, no `team restart` |
| "Who's authed to what?" | `ls ~/.openclaw/<group>/<name>/credentials/` per agent | Reads raw files; no CLI surface |
| "Which agents are connected to Telegram?" | `claws channel status` per agent | No matrix view |
| "Is there something running that I don't know about?" | `docker ps`, eyeball against `claws list` | Manual diff; bob situation |
| "Show me the audit log for just this group" | `claws access audit --since=...` (no filter) | All-instances stream, no group scope |
...
| "Is anyone close to RAM limit?" | `claws stats` (raw docker stats passthrough) | No alerting, no aggregation |
| "Bulk rotate tokens for this team" | Per-agent `token rotate` | No `team rotate` |
| "Tail logs across the team" | `claws logs <name> -f` per agent in separate terminals | No multi-agent tail, no `--group=` |
...
The single highest-value gap. `claws list` shows port/status/RAM/uptime; it should optionally show **identity** too.

**A1. `claws list --rich` (or `--wide`):**
```
...
**A3. `claws info <name>`** — the inverse: a single-agent deep-info command that consolidates `status` + `channel status` + `auth status` + `runtime show` + model + role + recent activity into one screen. Operators who want to drill in get one command instead of five. Cost: 3 hours.
...
**D1. `claws channels` (no args)** — fleet-wide channel matrix:
```
...
**D2. `claws auth status [name]`** — read what providers are configured per agent (no secrets shown). Read `credentials/` filenames + `agents.defaults.model.primary` to infer state:
```
...
**D3. `claws channels expiry <name>`** — for channels that can expire (WhatsApp Baileys sessions, Telegram bot tokens revoked upstream), surface "last successful event" so the operator sees a session is going bad *before* it fully fails. Cost: stretch — design first, defer.

### E. Drift detection — surface orphan/stale state (P1)

**E1. `claws orphans`** — find Docker containers matching `openclaw-*` whose project isn't in `~/.openclaw/.port-registry`. The bob case automatic.

```
$ claws orphans
ORPHAN                                    SINCE              MOUNTS                                   ACTION
openclaw-bob-openclaw-gateway-1           2026-05-20 03:45   /tmp/TestIntegration_.../bob (missing)   claws orphans clean openclaw-bob-...
```

With `claws orphans clean [name]` to `docker rm -f` it (with confirmation). Cost: 2 hours.

**E2. `claws orphans --reverse`** — registry entries whose container doesn't exist (created but never started, or container was nuked outside clawctl). Cost: 1 hour.

**E3. `claws drift` (umbrella)** — runs E1 + E2 + checks for instance dirs on disk without registry entries (and vice versa), surfaces stale `.port-registry.lock` files, etc. One command, one screen, do-no-harm. Cost: 3 hours after E1/E2.
...
**F1. `claws logs --group=<name> -f`** (covered by B) — interleaved tail.

**F2. `claws logs <name> --since=24h --grep=<pattern>`** — push the grep into the binary so operators don't have to pipe through bash and lose color codes.

**F3. `claws audit tail [-f]`** — read `.audit.log` with optional follow. Doesn't need anything new, just a UX layer over what's there.

**F4. `claws errors [--group=]`** — combined view of activity errors + audit "error" entries + container exit codes + restart counts. The "what just went wrong?" command.

Cost: F1 free with B; F2 = 2 hours; F3 = 1 hour; F4 = 3 hours.

### G. JSON parity — for embedding claws in your own dashboards (P2)
...
- `claws team rotate-tokens <team>`
- `claws team upgrade <team> --image=...`
- `claws team apply-policy <team>` (push policy to every member, restart affected)
- `claws team apply-config <team> <key> <value>` (set the same config across the team, e.g., `tools.profile`)
...
Adding the team scope is the difference between treating claws like a per-agent control surface (today) and treating it like a *team-level* control surface (where it's actually used). It also keeps claws honest about its scope: not Kubernetes, not multi-host, just *the right amount of orchestration for the one-server, small-team setup* — which is the positioning in the PMM report.
...
2. **A — fleet identity in `list --rich` + `claws info <name>`**. The single largest UX win. ~1 day.
3. **E1 — `claws orphans`**. Closes the bob class of bugs forever. ~2 hours.
...
- [ ] Run **one command** and see whether any container exists on the host that claws doesn't know about, with a one-command clean.
- [ ] Restart their whole team with one command — `claws team restart <name>` or `claws restart --group=<name>` — without writing a bash loop.
...
- [ ] Run `claws info <name>` and not need any other command to know what state that agent is in.

## Related

- `tickets/health-probe-loopback-bind-2026-05-23/ticket.md` — sibling: the deployed binary's *existing* `claws health` is currently broken under loopback bind. The work proposed here assumes `health` returns correct verdicts; if implemented before that fix, `--group=` filters on `health` will inherit the same false-negative.
...
- Multi-host orchestration: not in claws's positioning; deliberately not addressed.
- Web UI / dashboard server: the existing `claws dashboard` (live TUI) is enough for v0; HTML lives in the landing-page concern.
