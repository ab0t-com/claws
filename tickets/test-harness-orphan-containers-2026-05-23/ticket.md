...
1. **`TestIntegration_CreateInlineFlagsParsed`** — exercises `clawctl create bob --auth=codex ...`. The `--auth=codex` chain calls `cmdAuth codex` which runs `docker compose run --rm openclaw-cli ...`. `--rm` removes the CLI sidecar after exit, but **brings up the `openclaw-gateway` dependency** which has no `--rm`. The gateway keeps running with mounts pointing into `t.TempDir()`.

2. **`TestIntegration_StartGroupExpansion`** — exercises `clawctl start --group=alpha`. Real `docker compose up -d` per member. Container outlives `t.TempDir()`.
...
Surfaced repeatedly by `clawctl orphans` (ticket 10, Task E1) — that's how I noticed it during the 2026-05-23 audit. By the time the bob ticket was filed, my own session had created **3 more orphans** through the very tests that built `clawctl orphans`.
...
1. **Helper in `integration_test.go`**: extend the `clawctl(t, root, ...)` helper to track every `create` invocation's project name and register a single `t.Cleanup` that tears them all down. Cheap.

2. **Test-only `clawctl test-cleanup` subcommand**: a hidden command that walks the registry and `docker compose down -v`s every project. Tests call it in `t.Cleanup`. More general, more surface.
...
- [ ] `clawctl orphans` returns empty against any host that has just finished a clean test run.
...
- The 3 orphans currently on the host (`openclaw-alpha-one`, `openclaw-alpha-two`, `openclaw-bob`) were created by my session and are still there at the time of writing. The operator can clean them with `clawctl orphans clean --all --yes`. After this fix, that wouldn't have happened.
