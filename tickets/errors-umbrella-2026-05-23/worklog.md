...
**Goal.** One screen answering "what just went wrong, across the fleet?" — composing container state + recent log errors + recent clawctl audit errors + orphan containers, with a "Fix paths" trailer listing the exact commands to run.
...
2. **Fix paths are commands only, not commentary.** First draft included `# audit had: clawctl <cmd> ... (result=error at <ts>)` lines as "context." That was noise — operators reading "Fix paths" want commands they can copy-paste, not narrative. Stripped to just three categories: restart-loop diagnose, log-grep per affected instance, and orphan cleanup.
...
6. **Orphans are always global, never `--group=`-scoped.** Orphans are defined by their *absence* from clawctl's registry — they don't belong to any group. Scoping the orphans section to a group would mean "containers that don't exist in the registry but somehow we know they belong to this group," which is incoherent.
...
- Live read-only smoke against production fleet:
  ...
  Fix paths:
    clawctl logs team/sarah --grep=error --since=2h
...
- [x] `clawctl errors [--since=2h] [--group=<name>] [--json]` ships a single-screen umbrella.
...
Ticket 15 — `clawctl orphans --reverse` + `clawctl drift` umbrella. The last open ticket in this session's batch. Picking up.