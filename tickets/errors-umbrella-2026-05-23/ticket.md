# Ticket: `claws errors [--group=<name>]` — combined error/incident surface
...
1. `claws activity --since=2h` — log-error lines from each container
2. `claws access audit --since=2h` — claws operations that returned `error`
3. `docker ps -a` filtered to `exited` / `restarting` — container-runtime failures
4. `claws orphans` — containers running outside the registry's awareness

Each of these is one read. Composing them is the operator's job today. `claws errors` should do it.
...
==> Recent claws operations that returned error (1)
...
==> 3 instances need attention. Fix paths:
   claws logs team/sarah --grep=401 --since=2h    (whatsapp session needs re-login)
   claws orphans clean --all --yes                (remove leftover test containers)
```
...
**Is not**: an alerting system, a daemon, a metric collector. If an operator wants alerting they should script `claws errors --json | jq ...` into their existing pipeline.
...
- **Restart-count deltas**: today's claws can read `RestartCount` from `docker inspect`, but it shows lifetime restarts. To show "restarts in window", remember last-seen restart count between invocations — easiest place is a tiny `.errors-state.json` next to the registry. Or just show total restart count and let the operator judge.
- **Don't duplicate `claws activity`**. The activity command already shows recent file changes + log errors. `claws errors` should focus on the *abnormal* (errors, restarts, orphans), not the normal activity.

## Acceptance criteria

- [ ] `claws errors [--since=2h] [--group=<name>] [--json]` produces a single-screen summary across container state, container log errors, claws audit errors, and orphans.
...
- `tickets/auth-fleet-reauth-2026-05-23/` — `claws errors` should include "auth probe failed" once `auth status --probe` ships.
