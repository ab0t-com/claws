...
**Goal.** E2: `clawctl orphans --reverse` for registry entries without containers. E3: `clawctl drift` umbrella over E1 + E2 + filesystem-vs-registry mismatch.
...
| Custom runtime with non-default `projectPrefix` | Same scope limit as `clawctl orphans` — forward/reverse detection covers `openclaw-` prefix only. Documented in the original ticket |
...
- Live read-only smoke against production root:
  - `clawctl orphans --reverse` → "No reverse orphans found." (correct — every registered instance has a container)
  - `clawctl drift` → Forward orphans: 3 (the known test leftovers); reverse: 0; disk drift: 0; registry drift: 0. Fix paths suggested: `clawctl orphans clean --all --yes` (correct, single actionable command).

### Acceptance criteria from the ticket — status

- [x] `clawctl orphans --reverse` lists registry entries with no matching container.
- [x] `clawctl drift` runs all four checks and renders a single screen.
...
- [x] `clawctl errors` (ticket 14) was deliberately built before this and references this in its design — its orphans section is the same data as `drift`'s forward-orphan section. Consistency across the two umbrellas.
...
| OpenClaw `/__openclaw__/auth-check` endpoint | OpenClaw runtime team decides to expose it; clawctl strategy A flips from reserved to preferred |
...
| `clawctl drift fix --interactive` | If operators ask for it. Otherwise the manual fix-paths render stays correct |
