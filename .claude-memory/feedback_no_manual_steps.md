...
When a claws command changes system state, it must complete the full operation. No half-done states.
...
- Never require the user to run raw `docker compose` or `docker` commands to fix state claws created
