...
The user is building claws as a product — not just writing code. They think in terms of:
...
They expect claws to handle full lifecycle — if `group add` moves an instance, it should stop/start containers too. If a channel is added, one command should do config + restart + tell the user what's next. No loose ends.
