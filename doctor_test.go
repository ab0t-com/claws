...
func TestIntegration_Version(t *testing.T) {
	root := t.TempDir()
	out, err := clawctl(t, root, "version")
	if err != nil {
		t.Fatalf("version failed: %v\n%s", err, out)
	}
	if !strings.Contains(out, "clawctl") {
		t.Errorf("version should contain 'clawctl': %s", out)
	}
...
func TestIntegration_DoctorFix(t *testing.T) {
	...
	// Run doctor --fix
	out, _ := clawctl(t, root, "doctor", "--fix")
